...
import com.example.ecom.ViewModel.*
...
fun SignupScreen(modifier: Modifier=Modifier,navController: NavController,authViewModel: AuthViewModel = viewModel()){
    ...
    Column(modifier = Modifier.fillMaxSize().padding(8.dp).background(color = Color.White)
        ...
        Text("Hello There", modifier = Modifier.padding(top = 3.dp), fontSize = 40.sp, fontWeight = FontWeight.Bold)
        Spacer(modifier = Modifier.height(10.dp))
        Text("Create an account", modifier = Modifier, fontSize = 32.sp)
        ...
        Button(onClick = {
            ...
            authViewModel
            authViewModel.signup(email.value,name.value,password.value){ success,errorMessage->
                if(success){
